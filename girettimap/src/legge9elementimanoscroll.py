...
print("🔄 Scarico la pagina con impronta browser reale...")
try:
    # impersonate="chrome" inganna i filtri anti-bot
...
for idx, item in enumerate(items[::-1][:9]):
    try:
        ...
        if img_remote:
            ...
            # Download dell'immagine protetto con curl_cffi
            try:
                img_res = requests.get(img_remote, impersonate="chrome", timeout=10)
